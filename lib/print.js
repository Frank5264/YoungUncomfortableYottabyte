...
export default async function(m, client = {user: {}}) {
  const _name = await client.getName(m.sender);
  const sender = PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '');
  const chat = await client.getName(m.chat);
  let img;
  try {
    if (global.opts['img']) {
...
m.text ? m.text.length : 0) || 0;
  ...
  const me = PhoneNumber('+' + (client.user?.jid).replace('@s.whatsapp.net', '')).getNumber('international');
...
`.trim(),
  ...
  me + ' ~' + client.user.name + `${client.user.jid == global.client.user.jid ? '' : ' (Sub Bot)'}`,
...
m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, (v) => v.toUpperCase()) : '',
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) for (const user of m.mentionedJid) log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' +await client.getName(user)));
    console.log(m.error != null ? chalk.red(log) : m.isCommand ? chalk.yellow(log) : log);
  }
  if (m.messageStubParameters) {
    console.log(m.messageStubParameters.map((jid) => {
      jid = client.decodeJid(jid);
      const name = client.getName(jid);
