...
export function makeWASocket(connectionOptions, options = {}) {
  /**
     ...
     */
  const client = (global.opts['legacy'] ? makeWALegacySocket : _makeWaSocket)(connectionOptions);

  const sock = Object.defineProperties(client, {
    chats: {
      value: {...(options.chats || {})},
      writable: true,
    },
    ...
    sendNyanCat: {
      async value(jid, text = '', buffer, title, body, url, quoted, options) {
        if (buffer) {
          try {
            (type = await client.getFile(buffer), buffer = type.data);
          } catch {
            buffer = buffer;
          }
        }
	     const prep = generateWAMessageFromContent(jid, {extendedTextMessage: {text: text, contextInfo: {externalAdReply: {title: title, body: body, thumbnail: buffer, sourceUrl: url}, mentionedJid: await client.parseMention(text)}}}, {quoted: quoted});
    ...
    relayWAMessage: {
      async value(pesanfull) {
        ...
        const mekirim = await client.relayMessage(pesanfull.key.remoteJid, pesanfull.message, {messageId: pesanfull.key.id});
    ...
    sendFile: {
      ...
      async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
        const type = await client.getFile(path, true);
    ...
    sendContact: {
      ...
      async value(jid, data, quoted, options) {
        ...
        for (let [number, name] of data) {
          ...
          const biz = await client.getBusinessProfile(njid).catch((_) => null) || {};
          const vcard = `
BEGIN:VCARD
VERSION:3.0
N:;${name.replace(/\n/g, '\\n')};;;
...
X-WA-BIZ-NAME:${(client.chats[njid]?.vname || client.getName(njid) || name).replace(/\n/, '\\n')}
...
END:VCARD
    ...
    fakeReply: {
      ...
      value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options) {
        return client.reply(jid, text, {key: {fromMe: areJidsSameUser(fakeJid, client.user.id), participant: fakeJid, ...(fakeGroupJid ? {remoteJid: fakeGroupJid} : {})}, message: {conversation: fakeText}, ...options});
      },
    },
    downloadM: {
      /**
      ...
      async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
        const msg = proto.Message.fromObject({
          groupInviteMessage: proto.GroupInviteMessage.fromObject({
            ...
            groupJid: jid,
            groupName: (groupName ? groupName : await client.getName(jid)) || null,
            jpegThumbnail: Buffer.isBuffer(jpegThumbnail) ? jpegThumbnail : null,
            caption,
          }),
        });
        const message = generateWAMessageFromContent(participant, msg, options);
        await client.relayMessage(participant, message.message, {messageId: message.key.id, additionalAttributes: {...options}});
        return message;
      },
      enumerable: true,
    },
    ...
    ...(typeof client.chatRead !== 'function' ? {
      chatRead: {
        /**
                 * Read message
                 * @param {String} jid
                 ...
                 * @param {String} messageID
                 */
        value(jid, participant = client.user.jid, messageID) {
          return client.sendReadReceipt(jid, participant, [messageID]);
        },
        enumerable: true,
      },
    } : {}),
    ...(typeof client.setStatus !== 'function' ? {
      setStatus: {
        /**
                 * setStatus bot
                 * @param {String} status
 ...
 */
export function smsg(client, m, hasParent) {
  if (!m) return m;
  /**
     * @type {import('@whiskeysockets/baileys').proto.WebMessageInfo}
     */
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    sender: {
      get() {
        return this.client?.decodeJid(this.key?.fromMe && this.client?.user.id || this.participant || this.key.participant || this.chat || '');
      },
      enumerable: true,
    },
    fromMe: {
      get() {
        return this.key?.fromMe || areJidsSameUser(this.client?.user.id, this.sender) || false;
      },
    },
    mtype: {
      get() {
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {text: q} : q)), {
          ...
          fromMe: {
            get() {
              return areJidsSameUser(this.sender, self.client?.user.jid);
            },
            enumerable: true,
          },
          text: {
          ...
          reply: {
            ...
            value(text, chatId, options) {
              return self.client?.reply(chatId ? chatId : this.chat, text, this.vM, options);
            },
            enumerable: true,
          },
          copy: {
          ...
          forward: {
            ...
            value(jid, force = false, options) {
              return self.client?.sendMessage(jid, {
          ...
          delete: {
            /**
                         ...
                         */
            value() {
              return self.client?.sendMessage(this.chat, {delete: this.vM.key});
            },
            enumerable: true,
    ...
    download: {
      value(saveToFile = false) {
        const mtype = this.mediaType;
        return this.client?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ''), saveToFile);
      },
      enumerable: true,
      configurable: true,
    },
    reply: {
      value(text, chatId, options) {
        return this.client?.reply(chatId ? chatId : this.chat, text, this, options);
      },
    },
    copy: {
      value() {
    ...
    getQuotedObj: {
      value() {
        ...
        const q = proto.WebMessageInfo.fromObject(this.client?.loadMessage(this.quoted.id) || this.quoted.vM);
        return smsg(this.client, q);
      },
      enumerable: true,
    },
    getQuotedMessage: {
