...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const isROwner = [client.decodeJid(global.client.user.id), ...global.owner.map(([number]) => number)].map((v) => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender);
    ...
    const user = (m.isGroup ? participants.find((u) => client.decodeJid(u.id) === m.sender) : {}) || {}; // User Data
    const bot = (m.isGroup ? participants.find((u) => client.decodeJid(u.id) == this.user.jid) : {}) || {}; // Your Data
    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          /* for (const [jid] of global.reportes_solicitudes.filter(([number]) => number)) {
            const data = (await client.onWhatsApp(jid))[0] || {};
            if (data.exists) {
          ...
          fetch('https://themysticbot.cloud:2083/error', {
            ...
            body: JSON.stringify({number: client.user.jid, plugin: m.plugin, command: `${m.text}`, reason: format(e), md5: mddd5(md5c)}),
          });
        }
      }
      if (!opts['restrict']) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
          const chat = global.db.data.chats[m.chat];
          const user = global.db.data.users[m.sender];
          const botSpam = global.db.data.settings[mclient.client.user.jid];
...
╚═════════════════════╝
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mclient.client.reply(m.chat, `*[ ℹ️ ] Sus diamantes se han agotado, puede adquirir más con el comando: ${usedPrefix}buyall*`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mclient.client.reply(m.chat, `*[ ℹ️ ] Se require tener el nivel ${plugin.level} para poder utilizar el comando. Tú nivel actual es ${_user.level}, usa el comando ${usedPrefix}lvl para subir tu nivel con XP.*`, m);
          continue; 
        }
        const extra = {
          match,
          ...
          if (e) {
            ...
            if (e.name) {
              ...
              fetch('https://themysticbot.cloud:2083/error', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({number: client.user.jid, plugin: m.plugin, command: `${usedPrefix}${command} ${args.join(' ')}`, reason: text, md5: mddd5(md5c)}),
    ...
    const settingsREAD = global.db.data.settings[mclient.client.user.jid] || {};
    if (opts['autoread']) await mclient.client.readMessages([m.key]);
    if (settingsREAD.autoread2) await mclient.client.readMessages([m.key]);
  }
}
...
export async function participantsUpdate({id, participants, action}) {
  const m = mclient
  if (opts['self']) return;
  //if (m.client.isInit) return;
  if (global.db.data == null) await loadDatabase();
  const chat = global.db.data.chats[id] || {};
  const botTt = global.db.data.settings[m.client.user.jid] || {};
  let text = '';
  switch (action) {
    case 'add':
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        const groupMetadata = await m.client.groupMetadata(id) || (client.chats[id] || {}).metadata;
        for (const user of participants) {
          let pp = './src/avatar_contact.png';
          try {
            pp = await m.client.profilePictureUrl(user, 'image');
          } catch (e) {
          } finally {
            const apii = await m.client.getFile(pp);
            const antiArab = JSON.parse(fs.readFileSync('./src/antiArab.json'));
            const userPrefix = antiArab.some((prefix) => user.startsWith(prefix));
            const botTt2 = groupMetadata.participants.find((u) => m.client.decodeJid(u.id) == m.client.user.jid) || {};
            const isBotAdminNn = botTt2?.admin === 'admin' || false;
            text = (action === 'add' ? (chat.sWelcome || this.welcome || client.welcome || 'Welcome, @user!').replace('@subject', await m.client.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝚂𝙸𝙽 𝙳𝙴𝚂𝙲𝚁𝙸𝙿𝙲𝙸𝙾𝙽*') :
                              (chat.sBye || this.bye || client.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0]);
            if (userPrefix && chat.antiArab && botTt.restrict && isBotAdminNn && action === 'add') {
              const responseb = await m.client.groupParticipantsUpdate(id, [user], 'remove');
              if (responseb[0].status === '404') return;
              const fkontak2 = {'key': {'participants': '0@s.whatsapp.net', 'remoteJid': 'status@broadcast', 'fromMe': false, 'id': 'Halo'}, 'message': {'contactMessage': {'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${user.split('@')[0]}:${user.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}, 'participant': '0@s.whatsapp.net'};
              await m.client.sendMessage(id, {text: `*[❗] @${user.split('@')[0]} ᴇɴ ᴇsᴛᴇ ɢʀᴜᴘᴏ ɴᴏ sᴇ ᴘᴇʀᴍɪᴛᴇɴ ɴᴜᴍᴇʀᴏs ᴀʀᴀʙᴇs ᴏ ʀᴀʀᴏs, ᴘᴏʀ ʟᴏ ϙᴜᴇ sᴇ ᴛᴇ sᴀᴄᴀʀᴀ ᴅᴇʟ ɢʀᴜᴘᴏ*`, mentions: [user]}, {quoted: fkontak2});
              return;
            }
            await m.client.sendFile(id, apii.data, 'pp.jpg', text, null, false, {mentions: [user]});
          }
        }
      }
      break;
    ...
    case 'quitaradmin':
      if (!text) {
        text = (chat.sDemote || this.sdemote || client.sdemote || '@user ```is no longer Admin```');
      }
      text = text.replace('@user', '@' + participants[0].split('@')[0]);
      if (chat.detect && !chat?.isBanned) {
        mclient.client.sendMessage(id, {text, mentions: mclient.client.parseMention(text)});
      }
      break;
  }
}
...
export async function groupsUpdate(groupsUpdate) {
  ...
  for (const groupUpdate of groupsUpdate) {
    ...
    if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || client.sDesc || '```Description has been changed to```\n@desc').replace('@desc', groupUpdate.desc);
    if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || client.sSubject || '```Subject has been changed to```\n@subject').replace('@subject', groupUpdate.subject);
    if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || client.sIcon || '```Icon has been changed to```').replace('@icon', groupUpdate.icon);
    if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || client.sRevoke || '```Group link has been changed to```\n@revoke').replace('@revoke', groupUpdate.revoke);
    if (!text) continue;
    await mclient.client.sendMessage(id, {text, mentions: mclient.client.parseMention(text)});
  }
}

export async function callUpdate(callUpdate) {
  const isAnticall = global.db.data.settings[mclient.client.user.jid].antiCall;
  if (!isAnticall) return;
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await mclient.client.reply(nk.from, `Hola *@${nk.from.split('@')[0]}*, las ${nk.isVideo ? 'videollamadas' : 'llamadas'} no están permitidas, serás bloqueado.\n-\nSi accidentalmente llamaste póngase en contacto con mi creador para que te desbloquee!`, false, {mentions: [nk.from]});
        ...
        await mclient.client.sendMessage(nk.from, {contacts: {displayName: '𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑', contacts: [{vcard}]}}, {quoted: callmsg});
        await mclient.client.updateBlockStatus(nk.from, 'block');
      }
    }
  }
}
...
let date = d.toLocaleDateString('es', { day: 'numeric', month: 'long', year: 'numeric' })
 let time = d.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true })
    try {
        ...
        if (fromMe) return 
        let msg = mclient.client.serializeM(mclient.client.loadMessage(id))
...
*—◉ #disable antidelete*
┗━━━━━━━━━⬣  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀  ⬣━━━━━━━━━`.trim();
        await mclient.client.sendMessage(msg.chat, {text: antideleteMessage, mentions: [participant]}, {quoted: msg})
        mclient.client.copyNForward(msg.chat, msg).catch(e => console.log(e, msg))
    } catch (e) {
        console.error(e)
    }
}
...
global.dfail = (type, m, client) => {
  ...
  const aa = {quoted: m, userJid: client.user.jid};
  ...
  if (msg) return client.relayMessage(m.chat, prep.message, {messageId: prep.key.id});
};
...
watchFile(file, async () => {
  ...
  if (global.clients && global.clients.length > 0 ) {
    const users = [...new Set([...global.clients.filter((client) => client.user && client.ws.socket && client.ws.socket.readyState !== ws.CLOSED).map((client) => client)])];
